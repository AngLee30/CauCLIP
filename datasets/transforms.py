...
class GroupMultiScaleCrop(object):
    ...
    def __init__(self, input_size, scales=None, max_distort=1, fix_crop=True, more_fix_crop=True):
        ...
        self.input_size = input_size if not isinstance(input_size, int) else [input_size, input_size] # take input_size = 224 as an example: self.input_size is [224, 224]
        self.interpolation = Image.BILINEAR

    def __call__(self, img_group):

        im_size = img_group[0].size # (1280, 720)
        ...
        return ret_img_group # each PIL.Image.Image in ret_img_group has a size of (224, 224)
...
class Stack(object):
    ...
    def __call__(self, img_group):
        # img_group: list of PIL.Image.Image
        # an ndarray converted from a PIL.Image.Image has a shape of (224, 224, 3)
        if img_group[0].mode == 'L':
            ...
            if self.roll:
                ...
                rst = np.concatenate(img_group, axis=2) # ndarray: (224, 224, T*3)
                return rst
...
class ToTorchFormatTensor(object):
    ...
    def __call__(self, pic):
        if isinstance(pic, np.ndarray):
            # handle numpy array
            img = torch.from_numpy(pic).permute(2, 0, 1).contiguous() # (224, 224, T*3) -> (T*3, 224, 224)
        else:
            # handle PIL Image
            img = torch.ByteTensor(torch.ByteStorage.from_buffer(pic.tobytes()))
            img = img.view(pic.size[1], pic.size[0], len(pic.mode))
            img = img.transpose(0, 1).transpose(0, 2).contiguous()
        return img.float().div(255) if self.div else img.float() # divide each element in tensor by 255
